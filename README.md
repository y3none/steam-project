...
- [x] **跨视图联动可见化（`link-hints.js`）**
  - 监听 `EVT` 总线，联动触发时接收方视图脉冲高亮 + 「← 联动自 XX」提示 chip
  - 非侵入式，零改动现有联动逻辑
  - 涉及文件：`link-hints.js`(新)、`style.css`
...
- [ ] **优化移动端适配**
  - 散点图在窄屏下详情面板叠在底部体验差，改为弹出浮层或抽屉【待完成】
  - 衰减曲线图例在小屏上换行过多，考虑折叠/滚动【待完成】
...
- [x] **增加 scroll-driven 引导叙事**
  ...
  - 使用按钮防止霸占其余展示，生命周期： 首次滚动自动触发一次 → 之后用户随时可以通过按钮重播
...
- [x] **结论视图 View 05 · SYNTHESIS（三因一果）**  
  - 新增 section 与 `synthesis.js`，把「数量 / 质量 / 留存」三个发现收束成论点 + 收入结果
  - 全部数字由 `DATA` 实时计算（发布占比、顶尖好评率、24 月留存、每日新游等）
  - 加入「发现性危机」反转段，作为收尾论点
  - 入场动画：三张「因」卡错峰浮现 + 四个大数字滚动计数（`_synthesisReplay`）
  - 接独立 IntersectionObserver，普通滚动到达时也自动播放一次
  - 涉及文件：`synthesis.js`(新)、`index.html`、`style.css`

- [x] **自动导览 · 播放完整故事（`tour.js`）**
  - 常驻启动按钮 + 字幕条 + 进度点 + 播放/暂停/上一段/下一段/退出 + 键盘控制（空格 / ← → / Esc）
  - 6 章叙事：序章 → 数量 → 跨视图联动 → 质量 → 留存 → 终章
  - 复用各视图现有 narrative 接口，不重写图表逻辑
  - 进度点改为「轨道 + 绿色填充」，倒计时实时走条、暂停时冻结（解决「播放键无可见反馈」）
  - 尊重系统「减少动态效果」（reduced-motion 下瞬时到终态）
  - 涉及文件：`tour.js`(新)、`stream.js`(暴露 `_streamSelectYear`)、`style.css`
---

### 展示准备
- [x] **章节动态化（导览每章「活起来」）**
  - 01 数量：河流图沿时间线依次高亮 2012 → 2017 → 2020 → 回落 2017
  - 跨视图联动：年份在 2016 → 2018 → 2017 间擦动，散点图实时重筛
  - 02 质量：全量 → 淡出至只剩独立 → 点亮神作象限，三步引导
  - 03 留存：全部曲线 → 聚焦独立/3A → 时间游标扫描
  - 终章：因卡错峰浮现 + 大数字滚动计数
  - 时间游标扫描（`decay.js · _decaySweep`）：竖线从第 0 月扫到第 24 月，绿/红环标独立/3A 平均留存、中间竖条示差距、左上实时读数，差距随扫描肉眼拉开
  - 涉及文件：`decay.js`、`tour.js`、`synthesis.js`、`style.css`
