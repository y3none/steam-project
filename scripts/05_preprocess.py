...
def _generate_example_decay() -> list[dict]:
    ...
    profiles = [
        ...
        # ── 注意：name 必须与 bubbles 数据（Steam Store 名称）完全一致，否则联动按钮匹配不上 ──
        # AAA 单机：首发爆发 + 长尾衰减
        ("Cyberpunk 2077",           "AAA",  0.88, 0.72, 0.04, 0.03, "#ff9a9a", 1054388, 2020),
        ("Hogwarts Legacy",          "AAA",  0.92, 0.82, 0.03, 0.02, "#ffbaba", 879308, 2023),
        ("Black Myth: Wukong",       "AAA",  0.85, 0.55, 0.08, 0.06, "#ffe66d", 2416376, 2024),
        ("Baldur's Gate 3",          "AAA",  0.70, 0.24, 0.22, 0.16, "#ffd700", 875343, 2023),
        ("ELDEN RING",               "AAA",  0.88, 0.65, 0.06, 0.05, "#ff8080", 953426, 2022),
        ("Grand Theft Auto V Legacy","AAA",  0.90, 0.58, 0.05, 0.04, "#ff6b6b", 364548, 2015),
        # AA / 独立大作：衰减较缓
        ("Rust",                     "AA",   0.75, 0.35, 0.30, 0.25, "#ffc107", 245924, 2018),
        ("Warframe",                 "AA",   0.40, 0.08, 0.55, 0.48, "#ffab00", 145747, 2013),
        # 独立游戏：长尾特征明显
        ...
        # F2P / 持续运营：衰减极缓，长尾极长
        ("Counter-Strike 2",         "F2P",  0.20, 0.02, 0.78, 0.72, "#66bb6a", 1811425, 2012),
        ("Dota 2",                   "F2P",  0.28, 0.03, 0.68, 0.64, "#88ccaa", 1291328, 2013),
        ("PUBG: BATTLEGROUNDS",      "F2P",  0.82, 0.30, 0.45, 0.40, "#ff4c4c", 3257248, 2017),
        ("Apex Legends™",            "F2P",  0.80, 0.28, 0.50, 0.45, "#ff7c7c", 509744, 2019),
    ]
