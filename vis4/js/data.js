...
DATA.decay = [
  ...
  // AAA 单机
  {name:"GTA V",                   type:"AAA",color:"#ff6b6b",peak:364548, yr:2015,data:genDecay(1, 0.90,0.58,0.05,0.04)},
  ...
  // Indie
  {name:"Rust",                    type:"Indie",color:"#6dcbb0",peak:245924, yr:2018,data:genDecay(15,0.75,0.35,0.30,0.25)},  // 独立·自主发行(Facepunch)，按 Steam 标签与规则归 Indie
  ...
  // F2P / 持续运营
  {name:"CS:GO / CS2",             type:"F2P",color:"#66bb6a",peak:1811425,yr:2012,data:genDecay(18,0.20,0.02,0.75,0.70)},
  {name:"Dota 2",                   type:"F2P",color:"#88ccaa",peak:1291328,yr:2013,data:genDecay(10,0.28,0.03,0.68,0.64)},
  {name:"PUBG: BATTLEGROUNDS",      type:"F2P",color:"#4db89a",peak:3257248,yr:2017,data:genDecay(13,0.82,0.30,0.45,0.40)},
  {name:"Apex Legends™",            type:"F2P",color:"#6fc9a0",peak:509744, yr:2019,data:genDecay(16,0.80,0.28,0.50,0.45)},
];
...
window.backgroundRefresh = function() {
  ...
  fetch(API_BASE + '/refresh')
    ...
    .then(function(all) {
      ...
      if (window._synthesisRefresh) window._synthesisRefresh();  // 结论数字随热更新重算
...
window.refreshData = async function() {
  ...
  try {
    ...
    if (resp.ok) {
      ...
      // 重绘散点图（CCU 影响气泡位置）
      if (window._scatterRedraw) window._scatterRedraw();
      if (window._synthesisRefresh) window._synthesisRefresh();
